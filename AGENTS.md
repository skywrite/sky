...
- Consider the new-user experience in every new feature and UI/UX change. Account for users arriving with no data, configuration, or knowledge of Sky's conventions. Use helpful empty states and clear next actions to minimize time to a first useful result.
- Apply judgment about when onboarding needs to change. Address prerequisites where they become relevant—for example, guide users through connecting an account or providing an API key when a feature requires it. Features such as journaling or bios may warrant onboarding changes when they help users get value sooner. Keep setup focused and defer optional steps.
