...
Decisions live in `outbox/items/<conversation-hash>.md`. The body links to the canonical `me/voice/drafts/<id>.md` record through frontmatter `draftId`; frontmatter retains the situation, reasoning, questions, source snapshots, original draft, approved pairs, and native draft reference. Legacy inline drafts are adopted when opened or changed. The [shared draft contract](../../writingVoice/docs/README.md#shared-drafts-in-chat-and-outbox) owns naming, history, editing, learning, and concurrency with chat. Ordinary reads survive service restarts. Scanner metadata and locks live under the user-data state directory, scoped by notebook root. A damaged checkpoint is an error, not permission to reset the baseline and replay work.
