...
import {
  ...
  pickChannelByName,
} from '#commands/all/slack/lib/resolveChannelName.ts'
...
export default class SlackDraftNewTask extends Command {
  ...
  async run({ args, context }: CommandArgs<Params>): Promise<CommandResult<Result>> {
    const { env, output, systemNow } = context
    ...
    // thread here.
    let target = parseChannelTarget(args.conversation)?.channelId ?? args.conversation.trim()
    if (!target) {
      return CommandResult.fail(
        'Give a conversation: a Slack link, #channel, channel name, conversation id, or user id',
      )
    }

    // A name resolves here over the user token — agent-slack cannot look
    // names up on an Enterprise Grid (enterprise_is_restricted). Only a
    // unique exact name resolves; near matches come back as a question.
    if (!looksLikeConversationId(target) && env.SLACK_USER_TOKEN) {
      const pick = pickChannelByName(await fetchNamedChannels(new WebClient(env.SLACK_USER_TOKEN)), target)
      if (pick.kind === 'match') {
        output.log(colors.dim(`#${pick.channel.name} → ${pick.channel.id}`))
        target = pick.channel.id
      } else if (pick.kind === 'suggestions') {
        const names = pick.channels.map((c) => `#${c.name}`).join(', ')
        return CommandResult.fail(`No channel named "${target}" — close matches: ${names}. Say which one.`)
      } else {
        return CommandResult.fail(`No channel named "${target}" in the workspace.`)
      }
    }
