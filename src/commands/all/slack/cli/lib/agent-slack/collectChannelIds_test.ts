...
test('collectChannelIds: extracts bare channel mentions', () => {
  assert({
    ...
    actual: collectChannelIds([{ channel_id: 'C123', ts: '1.0', content: 'is <#C0123ATLASX> the right channel?' }]),
...
test('collectChannelIds: extracts labeled channel mentions', () => {
  assert({
    ...
    should: 'return the channel ID',
    actual: collectChannelIds([{ channel_id: 'C123', ts: '1.0', content: 'see <#C0123ATLASX|atlas-team>' }]),
    expected: ['C0123ATLASX'],
  })
})
...
test('collectChannelIds: deduplicates across messages', () => {
  assert({
    ...
    actual: collectChannelIds([
      { channel_id: 'C123', ts: '1.0', content: 'see <#C0123ATLASX>' },
      { channel_id: 'C123', ts: '2.0', content: 'again <#C0123ATLASX> and <#C0456ORIONX>' },
    ]).sort(),
...
test('collectChannelIds: ignores user mentions and plain text', () => {
  assert({
    ...
    actual: collectChannelIds([{ channel_id: 'C123', ts: '1.0', content: 'hey @U0123ABCDEF, no channels here' }]),
    expected: [],
  })
})
...
test('collectChannelIds: handles messages with no content', () => {
  assert({
    ...
    should: 'return empty array',
    actual: collectChannelIds([{ channel_id: 'C123', ts: '1.0' }]),
    expected: [],
  })
})
