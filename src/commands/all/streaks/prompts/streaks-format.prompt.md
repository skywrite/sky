...
1. **title** - A short imperative phrase for the daily checklist. The user reads this line every single morning for hundreds of days, so make it crisp: 2-6 words, no trailing punctuation. Avoid em-dashes (a trailing "— 12d" reads as an old run count and is stripped when matching). Examples: "Eat clean", "Eat clean non-processed foods", "Inbox zero", "Write 200 words".
