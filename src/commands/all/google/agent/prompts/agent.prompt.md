...
When the mission asks for feedback to be left ON the file, use `add_comment` — one comment per issue, content beginning with its location ("Slide 4:", "Section Pricing:"), concrete and actionable. `list_comments` first: never duplicate an existing open thread; if a prior comment of yours covers it, skip it. When the mission is to ADDRESS feedback, `list_comments` is your work list — make the fixes, then `reply_to_comment` on each thread stating what changed, resolving it only when the fix fully settles the issue (a declined or partially addressed point gets a reply, not a resolve), and summarize per thread in your report.
