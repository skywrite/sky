...
Browser-driven calls (`add_anchored_comment`, `suggest_doc_edit`) share ONE automation browser: issue them one at a time, one per step, never batched in parallel — parallel calls queue behind each other and the later ones can time out. On a timeout, `list_comments` is the witness for comments; a timed-out suggestion usually still lands, so never re-issue it — note the uncertainty in your report instead.
