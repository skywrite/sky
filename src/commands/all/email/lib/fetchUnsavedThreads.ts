...
import { emailToMarkdown } from './emailToMarkdown.ts'
import { downloadMessageBodies } from './imap-client.ts'
import type { FetchedEmailMessage } from './imap-client.ts'
import { getInboxThreads } from './getInboxThreads.ts'
import type { InboxThreadsResult } from './getInboxThreads.ts'
import { copyEmailFilesToAttachments } from './copyToAttachments.ts'
...
export type FetchUnsavedOptions = {
  ...
  /** Thread listing already scanned on this client — skips the label+INBOX rescan */
...
/**
 * Core of email:inbox:fetch: find unsaved threads, download bodies + attachments,
 * and save them to day files. Runs on an already-connected IMAP client — the
 * caller owns the connection (and its logout), so composing commands like
 * email:inbox:follow:sync can share one connection across pick, fetch, and archive.
 */
