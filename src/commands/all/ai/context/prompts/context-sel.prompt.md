...
- When the question names a topic, initiative, or theme, check the vocabulary BEFORE reaching for `bodyContains` — a tag filter finds the curated seam, not incidental word matches.
...
- "how did we run the 2023 rebrand" → Tag Vocabulary shows `Brand/… (9 tags, 41 files, last 2023-06)` → `documents(where: { tagsStartsWith: "Brand/" }, limit: 15) { type markdown path }` — a dormant branch is the seam for a history question
