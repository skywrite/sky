...
test('legal:annotate - rejects unusable documents without running a mission', async () => {
  const { context } = createContext()
  const { calls, tasks } = createTasks(agentSuccess())
  const task = new LegalAnnotateTask()
...
test('legal:annotate - sends a Google Doc URL as the mission file target', async () => {
  const { context } = createContext()
  const { calls, tasks } = createTasks(agentSuccess())
  const task = new LegalAnnotateTask()
...
test('legal:annotate - imports a local document and weights the brief by focus', async () => {
  const { context } = createContext()
  const { calls, tasks } = createTasks(agentSuccess())
  const task = new LegalAnnotateTask()
...
test('legal:annotate - passes a failed mission through', async () => {
  const { context } = createContext()
