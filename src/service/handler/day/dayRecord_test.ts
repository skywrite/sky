...
test({ name: 'day record - the plan and its outcome come from the day file' }, async () => {
  ...
  assert({
    given: 'a day file with Most Important, Commitments, Todos, and Complete sections',
    should: 'read each as items — done by strikethrough, category from the heading; capture logs are not Done',
    actual: {
      mostImportant: record.mostImportant.map((i) => i.text),
      commitments: record.commitments.map((i) => ({ text: i.text, done: i.done, category: i.category })),
    ...
    expected: {
      ...
      commitments: [
        { text: 'Send Jane the flat-floor math', done: true, category: 'Professional' },
        { text: 'Reply to the vendor shortlist by Friday', done: false, category: 'Professional' },
      ],
      todos: [{ text: 'Book the retreat holds', category: 'Personal' }],
      done: [{ text: 'Submit the expense report', done: true, category: 'Professional', time: null, link: null }],
    },
  })
})
...
test({ name: 'day record - meetings, messages, and journals come from what was filed' }, async () => {
  ...
  assert({
    given: 'a meeting, two messages, and a journal filed under the day',
    should: 'list the meeting by its heading with its time range and who, and the journal',
    actual: { meetings: record.meetings, journals: record.journals.map((j) => j.title), skipped: record.skipped },
    expected: {
      meetings: [
        {
          title: 'Atlas Sync',
...
test({ name: 'day record - a day with no file yet has an empty plan, not an error' }, async () => {
  ...
  assert({
    ...
    actual: {
      items: record.mostImportant.length + record.commitments.length + record.todos.length + record.done.length,
      filed:
