...
export function createOutboxHost(config: typeof Config, env: Record<string, string>): OutboxRoutesOptions {
  ...
  return {
    report: async () => {
      const [items, preferences, job, last] = await Promise.all([
        store.list(),
        store.preferences(),
        automation(),
    ...
    scan: async () => {
      ...
      return requireSuccess(await service().run('automations:run', { name: job.name, stamp: true }))
    },
    save: (id, revision, draft) => review.save(id, revision, draft),
    approve: (id, revision, draft, reviewedChanges) => review.approve(id, revision, draft, reviewedChanges),
