...
test({ name: 'approval card - a tool describes its own call; color codes do not reach the page' }, async () => {
  const lines = await approvalCard('slack_post', { channel: 'general', text: 'Hello' }, (input, output) => {
    output.log('')
...
test({ name: 'approval card - without a formatter the fields are listed, a long value on its own lines' }, async () => {
  assert({
    given: 'a call with a one-line and a multi-line field',
    should: 'list each field, the multi-line one under its key',
    actual: await approvalCard('notes_create', { title: 'Atlas', body: 'line one\nline two', tags: ['a', 'b'] }),
    expected: ['title: Atlas', 'body:', 'line one\nline two', 'tags: ["a","b"]'],
  })
})

test({ name: 'approval card - a call with nothing to show still names the tool' }, async () => {
  assert({
    given: 'an empty input and no formatter',
    should: 'fall back to the tool name so the card is never blank',
    actual: await approvalCard('day_items', undefined),
    expected: ['day_items'],
  })
})
