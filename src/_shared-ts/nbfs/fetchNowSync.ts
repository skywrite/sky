...
import { readDaySync } from '#shared/nbfs/mod.ts'
...
export interface FetchNowOptions {
  timeDir?: string
  now?: ZonedDateTime
}

export default function fetchNowSync(options: FetchNowOptions = {}): ZonedDateTime {
  ...
  try {
    dayModel = readDaySync(nowPlainDate, timeDir)
  ...
  while (!hasStarted && dayCounter < maxDaysToCheck) {
    ...
    try {
      dayModel = readDaySync(checkDate, timeDir)
      hasStarted = Boolean(dayModel.started)
    } catch (_e) {
