...
test('detectTypeFromPath - ai-chats paths are chats', () => {
  assert({
    ...
    actual: detectTypeFromPath('/nb/time/2026/02/02-08/02-03/actions/ai-chats/09-15_Planning-the-Widget-Launch.md'),
    expected: 'chat',
  })
})
...
test('detectTypeFromPath - open-day ai-chats paths are chats', () => {
  assert({
    given: 'an ai-chats path under a cross-month (MM-DD) day dir',
    should: 'detect type chat',
    actual: detectTypeFromPath('/nb/time/2026/06/29-05/07-04/actions/ai-chats/11-25_Some-Topic.md'),
    expected: 'chat',
  })
})
...
test('detectTypeFromPath - messages still detected alongside chat pattern', () => {
  assert({
    ...
    actual: detectTypeFromPath('/nb/time/2026/02/02-08/02-03/actions/messages/slack_Jane-to-Joe_Update.md'),
    expected: 'message',
  })
})
...
test('detectTypeFromPath - unmatched paths fall back to document', () => {
  assert({
    ...
    should: 'detect type document',
    actual: detectTypeFromPath('/nb/time/2026/02/02-08/02-03/meeting_jane-doe.md'),
    expected: 'document',
  })
})
