...
test('fixtures - slack message resolves from field to person', async () => {
  ...
  const messagePath = `${FIXTURES_DIR}time/2026/W04/01-23/actions/messages/slack_Sarah-Mitchell_Infrastructure-Budget-Discussion.md`
...
test('fixtures - day.md collects multiple meetings', async () => {
  const { store } = await loadFixtureStore()
  const dayContent = await loadFixture('time/2026/W04/01-23/day.md')
  const dayDoc = Document.fromMarkdown(dayContent)
  const dayPath = `${FIXTURES_DIR}time/2026/W04/01-23/day.md`
...
test('fixtures - fromDocuments combines multiple meetings', async () => {
  ...
  const meeting1Path = `${FIXTURES_DIR}time/2026/W04/01-23/actions/meetings/Zoom_Chen-Wei_Q1-Product-Roadmap-Review.md`
  const meeting2Path = `${FIXTURES_DIR}time/2026/W04/01-23/actions/meetings/Zoom_Marcus-Johnson_Investor-Relations-Update.md`
...
test('fixtures - deduplicates shared person across meetings', async () => {
  ...
  const meeting1Path = `${FIXTURES_DIR}time/2026/W04/01-23/actions/meetings/Zoom_Chen-Wei_Q1-Product-Roadmap-Review.md`
  const meeting2Path = `${FIXTURES_DIR}time/2026/W04/01-23/actions/meetings/FaceTime-Audio_Maria-Santos_Mobile-App-Redesign-Discussion.md`
...
test('fixtures - toMarkdown with relativeTo strips base path', async () => {
  ...
  assert({
    ...
    actual: output.includes('<!-- time/2026/W04/01-23/actions/meetings/Zoom_Chen-Wei_Q1-Product-Roadmap-Review.md -->'),
    expected: true,
  })
...
test('fixtures - journal health references project via rel', async () => {
  const { store } = await loadFixtureStore()
  const journalContent = await loadFixture('time/2026/W04/01-23/journal/01_health.md')
  const journalDoc = Document.fromMarkdown(journalContent)
  const journalPath = `${FIXTURES_DIR}time/2026/W04/01-23/journal/01_health.md`
...
test('fixtures - investor meeting resolves Northwind Ventures org', async () => {
  ...
  const meetingDoc = Document.fromMarkdown(meetingContent)
  const meetingPath = `${FIXTURES_DIR}time/2026/W04/01-23/actions/meetings/Zoom_Marcus-Johnson_Investor-Relations-Update.md`
...
test('fixtures - aggregate full day and write to output file', async () => {
  const { store } = await loadFixtureStore()
  const dayBase = 'time/2026/W04/01-23'
