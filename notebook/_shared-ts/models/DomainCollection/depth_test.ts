...
test('depth - root document gets depth 0', async () => {
  ...
  const content = await loadFixture('time/2026/01/20-26/23/actions/meetings/Zoom_Chen-Wei_Q1-Product-Roadmap-Review.md')
  const doc = Document.fromMarkdown(content)
  const path = `${FIXTURES_DIR}time/2026/01/20-26/23/actions/meetings/Zoom_Chen-Wei_Q1-Product-Roadmap-Review.md`
...
test('depth - person -> org chain gets depth 2', async () => {
  ...
  const doc = Document.fromMarkdown(content)
  const path = `${FIXTURES_DIR}time/2026/01/20-26/23/actions/meetings/Zoom_Marcus-Johnson_Investor-Relations-Update.md`
  ...
  const docs = [
    {
      ...
      path: `${FIXTURES_DIR}time/2026/01/20-26/23/actions/meetings/Zoom_Chen-Wei_Q1-Product-Roadmap-Review.md`,
    },
    {
      doc: Document.fromMarkdown(meeting2Content),
      path: `${FIXTURES_DIR}time/2026/01/20-26/23/actions/meetings/Zoom_Marcus-Johnson_Investor-Relations-Update.md`,
    },
  ]
