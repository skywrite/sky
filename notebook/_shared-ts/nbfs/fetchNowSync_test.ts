...
import fetchNowSync from './fetchNowSync.ts'
import { ZonedDateTime } from '#universal/dates/nbdt/mod.ts'
import dayFile from './dayFile.ts'

// Helper to create a temp directory with day files
function createTestDir(): string {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'fetchNowSync-test-'))
  return tmpDir
}
...
test('fetchNowSync - returns current day when day is started', () => {
  ...
  try {
    ...
    const result = fetchNowSync({ timeDir: testDir, now: mockNow })
...
test('fetchNowSync - finds previous started day when today not started', () => {
  const testDir = createTestDir()
...
test('fetchNowSync - throws when no day files exist', () => {
  const testDir = createTestDir()
...
test('fetchNowSync - calculates extended hours when past midnight', () => {
  const testDir = createTestDir()
